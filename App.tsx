    ...
    <div className="pb-24 animate-fade-in text-right">
      ...
      <div className="px-4 space-y-6">
        ...
        <div className="bg-white p-5 rounded-2xl shadow-sm">
          <h3 className="font-bold text-gray-800 mb-4 flex items-center gap-2 justify-end">
            استحقاقات قريبة/متأخرة <AlertCircle size={18} className="text-red-500" />
          </h3>
 ...
 const ClientsListView = () => {
    const clientsWithTotals = useMemo(() => {
      return data.clients.map(client => {
        ...
        // حساب أقرب تاريخ سداد متبقي (جديد)
    ...
    const filteredClients = useMemo(() => {
      return clientsWithTotals
        ...
        .sort((a, b) => {
          ...
          if (!aIsPaid && bIsPaid) return -1;
          // الترتيب حسب أقرب موعد سداد (جديد)
        ...
        <div className="bg-gray-50 min-h-screen pb-20 animate-fade-in text-right">
            ...
            <div className="p-4 space-y-4">
                <div className="bg-white p-5 rounded-2xl shadow-sm space-y-4">
                    ...
                    <div><label className="block text-xs text-gray-500 mb-1">ملاحظات (سبب التأجيل)</label><textarea value={notes} onChange={e => setNotes(e.target.value)} className="w-full p-3 bg-gray-50 border rounded-xl text-sm h-20 resize-none text-right" placeholder="سبب التأجيل، ملاحظات إضافية..." /></div>
                </div>
  ...
  const AddClientView = () => { // شاشة إضافة العميل المسترجعة
    ...
    return (
      <div className="bg-gray-50 min-h-screen pb-24 text-right animate-fade-in">
        ...
        <div className="p-4 space-y-5 text-right">
           ...
           <button onClick={() => saveDebt({ id: editingDebtId, clientId: selectedClientId, itemName, baseValue, profitPercentage, monthCount: months, startDate, paymentDay, installments: manualInstallments }, isEditMode)} className="w-full bg-blue-600 text-white py-4 rounded-xl font-bold shadow-lg shadow-blue-200 hover:bg-blue-700 transition-colors active:scale-95 flex items-center justify-center gap-2"><Save size={20} />حفظ المديونية</button>
        </div>
      </div>
    );
  };
      ...
      {isEditClientOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4">
          <div className="w-full max-w-md bg-white rounded-2xl shadow-xl p-4 text-right animate-fade-in">
            ...
            <div className="flex gap-2 mt-6"><button onClick={saveEditClient} className="flex-1 bg-blue-600 text-white py-3 rounded-xl font-bold active:scale-95 transition-transform">حفظ التغييرات</button><button onClick={() => setIsEditClientOpen(false)} className="flex-1 bg-gray-100 py-3 rounded-xl font-bold text-gray-600">إلغاء</button></div>
          </div>
        </div>
      )}
        ...
        <div className="fixed inset-0 z-[100] bg-black/60 flex items-end sm:items-center justify-center p-4">
          <div className="bg-white w-full max-w-md rounded-t-3xl sm:rounded-3xl p-6 shadow-2xl text-right animate-slide-up">
            ...
            <div className="flex gap-3">
              <button onClick={() => { window.open(`https://wa.me/${summaryPreview.phone}?text=${encodeURIComponent(summaryPreview.text)}`, '_blank'); setSummaryPreview(null); }} className="flex-1 bg-green-600 text-white py-4 rounded-2xl font-bold flex items-center justify-center gap-2 active:scale-95 transition-transform"><Send size={20}/> إرسال عبر واتساب</button>
