...
[cite_start]const INITIAL_DATA: AppData = { clients: [], debts: [] };
...
export default function App() {
  ...
  const [summaryPreview, setSummaryPreview] = useState<{ text: string; phone: string } | null>(null); // حالة المعاينة الجديدة
  ...
  // --- ACTIONS --- (تم الحفاظ عليها كما هي في ملفك الأصلي)
  const exportData = () => {
  ...
  const importData = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = (e) => {
      try {
        ...
        } else { alert('الملف غير صالح ❌'); } [cite: 17]
      } catch (err) { alert('حدث خطأ أثناء قراءة الملف ❌'); [cite_start]}
    };
  ...
  const saveDebt = (debtData: any, isEdit: boolean) => {
    ...
    if (isEdit) {
       setData(prev => ({ ...prev, debts: prev.debts.map(d => d.id === id ? { ...d, itemName, baseValue, profitPercentage, profitValue, totalValue: finalInstallmentsTotal, monthCount, startDate: new Date(startDate).getTime(), paymentDay, installments: processedInstallments } : d) })); [cite: 27-28]
    [cite_start]} else {
  ...
  const processPayment = (debtId: string, installmentId: string, paidAmount: number, paidDate: number, notes: string, newFutureInstallments: Installment[]) => {
    setData(prev => {
      const newDebts = prev.debts.map(debt => {
        ...
        const updatedCurrent: Installment = { ...currentInstallment, amount: paidAmount, status: isPostponed ? InstallmentStatus.POSTPONED : InstallmentStatus.PAID, paidDate: paidDate, notes: notes };
      ...
      <header className="bg-white p-6 pb-8 rounded-b-3xl shadow-sm mb-6">
        ...
        <div className="mt-6 grid grid-cols-2 gap-4">
          <div className="bg-blue-50 p-4 rounded-2xl border border-blue-100">
            <div className="flex items-center gap-2 mb-2 text-blue-600 justify-end">
          ...
          <div className="bg-green-50 p-4 rounded-2xl border border-green-100">
            <div className="flex items-center gap-2 mb-2 text-green-600 justify-end">
  ...
  const ClientsListView = () => {
    ...
    const filteredClients = useMemo(() => {
      return clientsWithTotals
        ...
        .sort((a, b) => {
          const aIsPaid = a.remaining <= 0; const bIsPaid = b.remaining <= 0;
          if (aIsPaid && !bIsPaid) return 1;
          if (!aIsPaid && bIsPaid) return -1;
          // الترتيب حسب أقرب موعد سداد (طلبك الجديد)
    ...
    return (
      <div className="pb-24 pt-4 px-4 h-full flex flex-col text-right">
        ...
        <div className="relative mb-6">
          <input type="text" placeholder="بحث..." className="w-full bg-white pl-4 pr-10 py-3 rounded-xl border-none shadow-sm text-sm focus:ring-2 focus:ring-blue-500 text-right" value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} /> [cite: 59-60]
          [cite_start]<Search className="absolute right-3 top-3 text-gray-400" size={20} />
        </div>
        <div className="space-y-3 overflow-y-auto no-scrollbar pb-20">
          {filteredClients.map(client => (
            <div key={client.id} onClick={() => { setSelectedClientId(client.id); setSelectedDebtIds([]); setCurrentView('CLIENT_DETAILS'); }} className={`bg-white p-4 rounded-xl shadow-sm active:scale-[0.99] transition-all cursor-pointer ${client.remaining <= 0 ? 'opacity-50 grayscale border-dashed border-gray-200' : ''}`}> [cite: 61-62]
              <div className="flex justify-between items-start">
                <div><h3 className={`font-bold ${client.remaining <= 0 ? [cite_start]'text-gray-400' : 'text-gray-800'}`}>{client.name}</h3>
                ...
                <div className="text-left">
                  ...
                  <span className={`font-bold ${client.remaining <= 0 ? 'text-gray-400' : 'text-red-500'}`}>{client.remaining <= 0 ? '0' : formatCurrency(client.remaining)}</span> [cite: 65-66]
                </div>
              </div>
            </div>
          ))}
  ...
  const ClientDetailsView = () => {
    [cite_start]const client = data.clients.find(c => c.id === selectedClientId);
    ...
    const handlePrepareSummary = () => {
      const selected = selectedDebtIds.length > 0 ? clientDebts.filter(d => selectedDebtIds.includes(d.id)) : clientDebts; [cite: 72-73]
      if (selected.length === 0) { alert('يرجى اختيار مديونية'); return; [cite_start]}
      ...
      [cite_start]text += `شكراً لتعاملك معنا.`;
      ...
      <div className="pb-24 bg-gray-50 min-h-screen animate-fade-in text-right">
        ...
        <div className="px-4 mt-6 space-y-6">
           <div className="flex justify-between items-center">
             ...
             <button onClick={() => { setEditingDebtId(null); setCurrentView('ADD_DEBT'); }} className="bg-blue-600 text-white px-4 py-2 rounded-xl text-sm font-bold shadow-lg shadow-blue-200">+ مديونية جديدة</button> [cite: 90-91]
           </div>
	         {clientDebts.map(debt => (
             [cite_start]<div key={debt.id} className={`bg-white rounded-2xl shadow-sm overflow-hidden border transition-all ${selectedDebtIds.includes(debt.id) ? 'border-blue-500 ring-1 ring-blue-500' : 'border-gray-100'}`}> [cite: 91-92]
               <div className="p-4 border-b border-gray-100 bg-gray-50/50 flex justify-between items-start">
                 <div className="flex items-center gap-3">
                   [cite_start]<input type="checkbox" checked={selectedDebtIds.includes(debt.id)} onChange={() => toggleDebtSelection(debt.id)} className="w-5 h-5 rounded border-gray-300 text-blue-600 focus:ring-blue-500 cursor-pointer" />
                   <div className="text-right">
                     <h4 className="font-bold text-gray-900">{debt.itemName}</h4>
                     <p className="text-xs text-gray-500 mt-1">أصل: {formatCurrency(debt.baseValue)} | ربح: {debt.profitPercentage.toFixed(1)}%</p> [cite: 94-95]
                   </div>
                 </div>
                 <div className="flex gap-2">
                   <button onClick={() => { setEditingDebtId(debt.id); setCurrentView('EDIT_DEBT'); [cite_start]}} className="p-2 bg-gray-200 text-gray-600 rounded-lg hover:bg-gray-300"><Edit size={16} /></button>
               ...
               <div className="divide-y divide-gray-100">
                 {debt.installments.map((inst, idx) => (
                   <div key={inst.id} className="p-4 flex items-center justify-between hover:bg-gray-50 transition-colors">
                     <div className="flex items-center gap-3">
                       <span className={`w-6 h-6 rounded-full flex items-center justify-center text-xs font-medium ${inst.status === 'PAID' ? 'bg-green-100 text-green-700' : inst.status === InstallmentStatus.POSTPONED ? 'bg-orange-100 text-orange-700' : 'bg-gray-100 text-gray-500'}`}>{idx + 1}</span> [cite: 97-98]
                       [cite_start]<div className="text-right"><p className="text-sm font-medium text-gray-900">{formatCurrency(inst.amount)}</p><p className="text-xs text-gray-500">{formatDate(inst.dueDate)}</p></div>
                     </div>
                     {inst.status === InstallmentStatus.PAID || inst.status === InstallmentStatus.POSTPONED ? ( [cite: 98-99]
                       <div className="flex items-center gap-2">
                         [cite_start]{inst.status === InstallmentStatus.PAID && (<button onClick={() => sendInstallmentReceipt(debt, inst, idx)} className="text-green-600 bg-green-50 p-1.5 rounded-md hover:bg-green-100"><Receipt size={16} /></button>)} [cite: 99-100]
                         <button onClick={() => inst.status === InstallmentStatus.POSTPONED && setPostponedInfo({ date: inst.dueDate, note: inst.notes || '' })} className={`text-xs font-bold px-2 py-1 rounded-md ${inst.status === InstallmentStatus.POSTPONED ? 'bg-orange-50 text-orange-600' : 'bg-green-50 text-green-600'}`}>{inst.status === InstallmentStatus.POSTPONED ? [cite_start]'تم التأجيل' : 'مدفوع'}</button>
                       </div>
                     ) : (
  ...
  const RecordPaymentView = () => {
    ...
    const bal = totalDebt - previouslyPaid - paymentAmount;

    useEffect(() => {
        if (bal <= 0 && remainingMonths <= 0) { setPreviewInstallments([]); return; }
        const safeM = (bal > 1 && remainingMonths === 0) ? 1 : remainingMonths;
        if (safeM > 0) {
            ...
            const plan = calculatePlan(Math.max(0, bal), safeM, nextStartDate, debt.paymentDay);
            setPreviewInstallments(plan.map(p => ({ ...p, id: generateId(), debtId: debt.id, status: InstallmentStatus.PENDING })));
        }
    }, [paymentAmount, remainingMonths, paymentDate, bal, debt]);
    ...
    return (
        <div className="bg-gray-50 min-h-screen pb-20 animate-fade-in text-right">
            ...
            <div className="p-4 space-y-4">
                ...
                <button onClick={() => processPayment(debt.id, installment.id, paymentAmount, new Date(paymentDate).getTime(), notes, previewInstallments)} className="w-full bg-blue-600 text-white py-4 rounded-xl font-bold shadow-lg">{paymentAmount === 0 ? 'تأكيد التأجيل والجدولة' : 'تأكيد السداد والجدولة'}</button> [cite: 126-127]
            </div>
        </div>
    );
  };
  ...
  const AddClientView = () => {
    [cite_start]const [name, setName] = useState('');
  ...
  const DebtFormView = () => { // إعادة لوحة التعديل كما كانت في ملفك الأصلي تماماً
    ...
    const client = data.clients.find(c => c.id === (existingDebt?.clientId || selectedClientId)); [cite: 134-135]
    [cite_start]const [itemName, setItemName] = useState(existingDebt?.itemName || '');
    ...
    const handleBaseValueChange = (val: number | '') => {
        setBaseValue(val); const base = Number(val) || 0; [cite: 139-140]
        if (base > 0) { if (profitType === 'PERCENTAGE') setFixedProfit(base * (profitPercentage / 100)); else setProfitPercentage(((Number(fixedProfit) || 0) / base) * 100); [cite_start]} [cite: 140-141]
    };
    const handleFixedProfitChange = (val: number | '') => { 
        setFixedProfit(val); const base = Number(baseValue) || 0; const fixed = Number(val) || [cite_start]0; [cite: 143-144]
        [cite_start]if (base > 0) setProfitPercentage((fixed / base) * 100);
    };
    const getCalculatedValues = () => { 
        const base = Number(baseValue) || 0; let profit = 0; [cite: 146-147]
        if (profitType === 'PERCENTAGE') profit = base * (profitPercentage / 100); else profit = Number(fixedProfit) || [cite_start]0; [cite: 147-148]
        [cite_start]return { base, profit, total: base + profit };
    };
    const handleRecalculate = () => { // الزر لإعادة إنشاء الجدول عند الحاجة فقط
        const { total } = getCalculatedValues(); if (total === 0) return; [cite: 149-150]
        [cite_start]const plan = calculatePlan(total, months, new Date(startDate), paymentDay);
    ...
    const updateInstallment = (index: number, field: 'amount' | 'dueDate', value: any) => { // دالة التعديل اليدوي المسترجعة
        const newInst = [...manualInstallments];
        if (field === 'dueDate') { newInst[index].dueDate = new Date(value).getTime(); setManualInstallments(newInst); } [cite: 154]
        else {
            [cite_start]const newAmount = Number(value); if (newAmount < 0) return; [cite: 155-156]
            [cite_start]newInst[index].amount = newAmount; const rem = newInst.length - 1 - index;
            if (rem > 0) {
                ...
                for (let j = index + 1; j < newInst.length; j++) { const isLast = j === newInst.length - 1; newInst[j].amount = Math.max(0, amtPerM + (isLast ? r : 0)); } [cite: 159-160]
            }
            [cite_start]setManualInstallments(newInst);
        }
    };
      ...
      <div className="bg-gray-50 min-h-screen pb-24 text-right animate-fade-in">
        ...
        <div className="p-4 space-y-5">
           <div className="bg-white p-4 rounded-xl shadow-sm space-y-4">
             ...
             <div><label className="block text-xs text-gray-500 mb-1">رأس المال</label><input type="number" value={baseValue} onChange={e => handleBaseValueChange(e.target.value === '' ? '' : Number(e.target.value))} className="w-full p-3 bg-gray-50 rounded-lg border font-bold text-right" /></div> [cite: 165-166]
             <div className="bg-gray-50 p-3 rounded-xl border border-gray-200">
                <div className="flex bg-white rounded-lg p-1 shadow-sm mb-3">
                   <button onClick={() => setProfitType('PERCENTAGE')} className={`flex-1 py-1.5 text-xs font-bold rounded-md flex items-center justify-center gap-1 ${profitType === 'PERCENTAGE' ? [cite_start]'bg-blue-100 text-blue-700' : 'text-gray-500'}`}><Percent size={14} /> نسبة</button>
           ...
           <div className="bg-white p-4 rounded-xl shadow-sm space-y-4">
             ...
             <div className="mt-4">
                <div className="flex justify-between items-center mb-2"><span className="text-sm font-bold">جدول الأقساط</span><span className={`font-bold ${currentTotal === targetTotal ? 'text-green-600' : 'text-red-500'}`}>{formatCurrency(currentTotal)}</span></div> [cite: 172-173]
                <div className="max-h-80 overflow-y-auto border rounded-lg divide-y bg-gray-50">
                    {manualInstallments.map((inst, idx) => {
                        [cite_start]const isLast = idx === manualInstallments.length - 1;
                ...
                <p className="text-[10px] text-gray-400 mt-2 text-center">* يمكنك تعديل المبلغ والتاريخ يدوياً. القسط الأخير يحسب تلقائياً.</p> [cite: 173-174]
             </div>
           </div>
           <button onClick={() => saveDebt({ id: editingDebtId, clientId: client.id, itemName, baseValue, profitPercentage, monthCount: months, startDate, paymentDay, installments: manualInstallments }, isEditMode)} className="w-full bg-blue-600 text-white py-4 rounded-xl font-bold shadow-lg active:scale-95 transition-transform"><Save size={20} className="inline ml-2" /> {isEditMode ? [cite_start]'حفظ التعديلات' : 'اعتماد المديونية'}</button>
        </div>
      </div>
    );
  };
  ...
  const SettingsView = () => (
    <div className="bg-gray-50 min-h-screen pb-24 text-right animate-fade-in">
      ...
      <div className="px-4 space-y-6">
        <button onClick={exportData} className="w-full flex items-center justify-between p-4 bg-blue-50 text-blue-700 rounded-xl font-bold shadow-sm active:bg-blue-100 transition-colors"><Download size={20} /> تصدير نسخة احتياطية</button>
        <label className="w-full flex items-center justify-between p-4 bg-green-50 text-green-700 rounded-xl font-bold cursor-pointer shadow-sm active:bg-green-100 transition-colors"><Upload size={20} /> استيراد بيانات<input type="file" accept=".json" onChange={importData} className="hidden" /></label>
  ...
  return (
    <>
      {isEditClientOpen && ( // نافذة تعديل العميل المسترجعة
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4">
          <div className="w-full max-w-md bg-white rounded-2xl shadow-xl p-4 text-right animate-fade-in">
            <div className="flex items-center justify-between mb-4">
                ...
                <button onClick={() => setIsEditClientOpen(false)} className="p-2 rounded-full bg-gray-100 hover:bg-gray-200">✕</button>
            </div>
            ...
            <div className="flex gap-2 mt-6"><button onClick={saveEditClient} className="flex-1 bg-blue-600 text-white py-3 rounded-xl font-bold active:scale-95 transition-transform">حفظ</button><button onClick={() => setIsEditClientOpen(false)} className="flex-1 bg-gray-100 py-3 rounded-xl font-bold text-gray-600">إلغاء</button></div> [cite: 180-181]
          </div>
        </div>
      )}

      <div className="max-w-md mx-auto bg-gray-50 min-h-screen relative shadow-2xl overflow-hidden font-tajawal">
        [cite_start]{currentView === 'DASHBOARD' && <DashboardView />}
      ...
      {postponedInfo !== null && ( // نافذة تفاصيل التأجيل الأصلية
        <div onClick={() => setPostponedInfo(null)} className="fixed inset-0 z-[999999] bg-black/45 flex items-center justify-center text-right">
          <div onClick={(e) => e.stopPropagation()} className="bg-white rounded-[14px] p-[18px] w-[90%] max-w-[420px] shadow-2xl text-center"> [cite: 184-185]
            [cite_start]<div style={{color:'#FF7700' , fontWeight: 800, marginBottom: 6 }}>تفاصيل التأجيل</div>
            ...
            <button onClick={() => setPostponedInfo(null)} style={{marginTop: 14, padding: '10px 16px', borderRadius: 12, background: '#FF7700', color: '#fff', fontWeight: 700, width: '100%'}}>إغلاق</button> [cite: 186-187]
          </div>
        </div>
      )}
    </>
