...
export default function App() {
  ...
  // خاصية اختيار المديونيات لكشف الحساب
            ...
            return (
              <div 
                ...
                onClick={() => { 
                  ...
                  setSelectedDebtIds([]); // تصفير الاختيارات عند فتح ملف عميل جديد
                  setCurrentView('CLIENT_DETAILS'); 
                }} 
          ...
          <div className="flex flex-col items-center">
            ...
            <div className="flex gap-3 mt-4 w-full justify-center">
               <button 
                 ...
                 className={`flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-medium transition-colors ${selectedDebtIds.length > 0 ? 'bg-green-600 text-white shadow-lg' : 'bg-green-50 text-green-700 hover:bg-green-100'}`}
               >
                 <Send size={16} /> 
                 {selectedDebtIds.length > 0 ? `إرسال كشف (${selectedDebtIds.length})` : 'إرسال كشف الحساب'}
               </button>
            </div>
          </div>
        </div>
        <div className="px-4 mt-6 space-y-6">
           <div className="flex justify-between items-center">
             <h3 className="font-bold text-gray-800">
               {selectedDebtIds.length > 0 ? `تم اختيار (${selectedDebtIds.length}) مديونية` : 'الديون المسجلة'}
             </h3>
