...
export default function App() {
  ...
  const [summaryPreview, setSummaryPreview] = useState<{ text: string; phone: string } | null>(null); // حالة معاينة الكشف
      ...
      <div className="px-4 space-y-6">
        <div className="bg-white p-5 rounded-2xl shadow-sm text-right">
          <h3 className="font-bold text-gray-800 mb-4 flex items-center gap-2 justify-end">
            تحليل الأداء <TrendingUp size={18} className="text-gray-400" />
          </h3>
  ...
  const ClientsListView = () => {
    const clientsWithTotals = useMemo(() => {
      return data.clients.map(client => {
        ...
        // حساب أقرب تاريخ سداد قادم للترتيب
    ...
    return (
      <div className="pb-24 pt-4 px-4 h-full flex flex-col text-right">
        ...
        <div className="relative mb-6">
          <input type="text" placeholder="بحث..." className="w-full bg-white pl-4 pr-10 py-3 rounded-xl border-none shadow-sm text-sm text-right focus:ring-2 focus:ring-blue-500" value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} />
  ...
  const RecordPaymentView = () => {
    ...
    useEffect(() => {
        ...
        if (safeM > 0) {
            const plan = calculatePlan(Math.max(0, bal), safeM, new Date(paymentDate), debt.paymentDay);
    ...
    return (
        <div className="bg-gray-50 min-h-screen pb-20 animate-fade-in text-right">
            ...
            <div className="p-4 space-y-4">
                ...
                <button onClick={() => processPayment(debt.id, installment.id, paymentAmount, new Date(paymentDate).getTime(), notes, previewInstallments)} className="w-full bg-blue-600 text-white py-4 rounded-xl font-bold shadow-lg shadow-blue-200">{paymentAmount === 0 ? 'تأكيد التأجيل' : 'تأكيد السداد'}</button>
            </div>
        </div>
    );
  };
    ...
    return (
      <div className="bg-white min-h-screen text-right animate-fade-in">
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-6">
           ...
           <button type="submit" className="w-full bg-blue-600 text-white py-4 rounded-2xl font-bold text-lg mt-10 shadow-lg active:scale-95 transition-transform">حفظ العميل الجديد</button>
        </form>
      </div>
    );
  };
    ...
    <>
      {isEditClientOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4">
          <div className="w-full max-w-md bg-white rounded-2xl shadow-xl p-6 text-right animate-fade-in">
            <h3 className="font-bold mb-4 text-lg">تعديل بيانات العميل</h3>
            ...
            <div className="flex gap-2 mt-6"><button onClick={saveEditClient} className="flex-1 bg-blue-600 text-white py-3 rounded-xl font-bold active:scale-95 transition-transform">حفظ</button><button onClick={() => setIsEditClientOpen(false)} className="flex-1 bg-gray-100 py-3 rounded-xl font-bold">إلغاء</button></div>
          </div>
        </div>
      )}
      ...
      {/* نافذة معاينة كشف الحساب الجديدة */}
      {summaryPreview && (
